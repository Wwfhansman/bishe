...
def test_detector_process_audio_emits_pending_and_confirmed_events():
    ...
    detector.state.early_trigger_min_frames = 99
...
def test_detector_process_audio_emits_early_trigger_before_confirmed():
    detector = AdaptiveBargeInDetector()
    detector.state.speech_state = SpeechState.SPEAKING
    detector.state.early_trigger_min_frames = 1
    detector.state.start_frames = 99
    detector.state.pending_score = 0.5
    detector.state.confirm_score = 10.0
    detector.state.noise_floor = 100.0
    ...
    events = detector.process_audio(
        ...
        last_tts_chunk_time=time.time() - 2.0,
        last_tts_end_time=time.time() - 2.0,
        tts_start_time=time.time() - 0.5,
    )

    assert events[0].kind == BargeInEventKind.EARLY_TRIGGER
    assert detector.state.speech_state == SpeechState.INTERRUPT_PENDING
